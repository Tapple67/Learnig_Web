...
export async function POST(req: Request) {
  ...
  if (!form) return NextResponse.json({ error: "formData를 읽지 못했습니다." }, { status: 400 });
  ...
  if (!subjectId) return NextResponse.json({ error: "subjectId가 필요합니다." }, { status: 400 });
  if (!Number.isFinite(week) || week < 1) return NextResponse.json({ error: "week가 올바르지 않습니다." }, { status: 400 });
  ...
  const material = await prisma.material.create({
    data: { subjectId: subject.id, week, title, fileUrl },
    select: { id: true, week: true, title: true, fileUrl: true },
  });
